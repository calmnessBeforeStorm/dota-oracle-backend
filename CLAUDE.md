...
- Рейтинги игроков (TrueSkill по `account_id`) пересчитываются строго хронологически:
  рейтинг, записанный на матч, — это состояние **до** него (`ratings`).
- **Ещё нет:** драфт-подмодели (§6.3) и pre-match признаков на базе рейтингов.
...
1. **Никакой информации из будущего в признаках.** Всё считается point-in-time. Пример
   ...
   Рейтинги игроков — тот же принцип во времени: `rebuild_player_ratings` идёт по матчам
   в хронологическом порядке и пишет рейтинг **до** матча, обновляя его только после
   записи. Пересборка всегда полная: приход старого матча из бэкфилла делает недействительным
   всё, что после него.
