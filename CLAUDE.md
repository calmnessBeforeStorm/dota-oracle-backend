...
**Фазы 1–3** по дорожной карте §11, идут сейчас.
...
- **Ещё нет:** point-in-time рейтингов игроков (§6.2) и драфт-подмодели (§6.3).
...
укладываются в одну месячную квоту OpenDota, но это ~46 часов закачки: скорость деталей
ограничена не rate limit-ом, а временем ответа — ~16,5 карт в минуту, не 60 (врезка §2.2/A4).
...
10. **`/matches/{id}` не знает о сериях.** `series_id` и `series_type` приходят оттуда
    `null` — принадлежность к серии владеет только слой сводок `/proMatches`, и разбор
    деталей не имеет права её перезаписывать.
11. **Неизвестное хранится как NULL, а не как значение по умолчанию.** `series.format` и
