...
from app.ingestion.liquipedia.matching import MappingProposal, best_proposal
...
async def _categories_of(client: LiquipediaSource, titles: list[str]) -> dict[str, list[str]]:
    """Categories for several pages in one request."""
    if not titles:
        return {}
    response = await client.query(prop="categories", titles="|".join(titles), cllimit=500)
    pages = (response.get("query") or {}).get("pages") or {}
    return {
        page["title"]: [c["title"] for c in page.get("categories") or []]
...
async def propose_mappings(
    ...
    for league_id, name in leagues:
        ...
        categories = await _categories_of(client, titles)
        candidates = [(title, categories.get(title, [])) for title in titles]

        proposal = best_proposal(league_id, name, candidates)
